<template>
  <section class="component inner-rank-list">
    <el-card class="box-card">
      <div slot="header" class="card-header row between-span">
        <span>催收数据</span>
        <div class="row end-span middle-span">
          <rank-type-select @onRankTypeChange="refreshData" v-model="innerCaseModel.rankType"></rank-type-select>
        </div>
      </div>
      <data-box :height="300" :data="innerDataSet" @onPageChange="refreshData" :page="pageService" hiddenIndex>
...
export default class InnerRankList extends Vue {
  @Dependencies(PageService) 
  private pageService: PageService;

  private innerDataSet: any = null;

  private innerCaseModel = {
