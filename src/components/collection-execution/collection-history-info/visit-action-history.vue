  ...
  <section class="component visit-action-history">
    ...
    <data-box @onPageChange="refreshData" :data="dataSet" :page="pageService" :height="200">
      <template slot="columns">
        ...
        <el-table-column prop="target" label="与本人关系" :min-width="$helper.getColumnWidth(4)" :formatter="(row) => $filter.dictConvert(row.target, 'Relationship')">
        </el-table-column>
        <el-table-column prop="targetName" label="姓名" :min-width="$helper.getColumnWidth(4)">
        </el-table-column>
        <el-table-column prop="addressDetail" label="催收地址" :min-width="$helper.getColumnWidth(8)">
        </el-table-column>
