...
export default class OriganizeTree extends Vue {
  ...
  private createTree(data) {
    ...
    const fun = (id) => {
      // 递归对象子元素
      const list = data.filter((x) => x.parentId === id).map((node) => {
        // 递归构建子节点
        const children = fun(node.id);
        // 添加icon标识
        node.isIcon = true;
        if (!this.isIcon) {
          node.isIcon = false;
    ...
    const parent = data.find((x) => x.id === this.userData.deptId);
    let flag = true;
    if (!parent) {
      flag = false;
    }
