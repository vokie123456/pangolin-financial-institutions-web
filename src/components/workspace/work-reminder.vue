...
export default class WorkReminder extends Vue {
  ...
  private refreshData() {
    this.reminderMessageService.getWorkbenchReminder().subscribe(
      data => {
  ...
  private mounted() {
    this.refreshData();
    // 收到消息通知时更新未读消息
    this.$reminder.addMessageListener().subscribe(data => {
      this.refreshData();
    });
  }
}
