  ...
  <section class="page assist-visit-wait-distribution">
    <data-form :model="searchModel" @onSearch="refreshData">
      ...
      <template slot="default-button">
        <el-button @click="manualDivision">分案</el-button>
      </template>
    </data-form>
    ...
    <el-dialog title="分案" :visible.sync="dialog.ManualDivision" width="800px" :close-on-click-modal="false" :close-on-press-escape="false" @open="$nextTick(()=>$refs['user-case-allot'].refresh(cheatDepartmentDataNoRoot,collectionType))">
...
export default class AssistCisitWaitDistribution extends Vue {
  ...
  private collectionType = 'CHEAT_STAY';
  ...
  private refreshData() {
    this.caseAssistAntiFraudCaseStayService
      .queryAntiFraudList(this.searchModel, this.pageService, this.sortService)
      .subscribe(
