const createDataInfoExcelService = () => import("~/services/dataimp-service/data-import-excel-info.service")
export default {
  namespaced: true,
  state: {
  ...
  actions: {
    ...
    async getBatchList({ state, commit }) {
      const { DataImportExcelInfoService } = await createDataInfoExcelService()
      DataImportExcelInfoService.
        getInstance()
        .getAllBatchNumbers().subscribe(data => {
